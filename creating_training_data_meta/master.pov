...
#local cam_dist = 50;                   //50 ---- 75
#local cam_move = 1/2;
#local cam_area = 2;
#local cam_loca = -z * cam_dist;
#local cam_look = <0,0,0>;
#local cam_angl = 45;
#local cam_tran = transform
{
	rotate		+x * asind(tand(30))    //0 ( horizontal ) ---- 45 (vertical)
	rotate		+y * 45               //can be anything (degress)
...
light_source {
    <2, 10, -3>,
    rgb <1,1,1>                             //RGB Vector with 3 values between 0.8 and 1.0
    area_light <5, 0, 0>, <0, 0, 5>, 2, 2     //How many x lights, and how many y lights (integers)
    adaptive 1
    jitter
  }
...
background { color rgb <0,0,0> }             //RGB Vector with 3 values
...
#local brick_model = object { 
        ...
        translate <0,0,0>    //XYZ Vecotr (Y is 0.96 for normal brick height)
        texture { pigment { color rgb <1,0.5,1> }}     // RGB Vector with 3 values
        }
...
object {
    plane { y, 0}
    texture {
        pigment { color rgb <0.5, 1, 1> }        //RGB Vector with 3 values
        finish { ambient 0 diffuse 0.8 }     //Diffusion value between 0.4 and 0.8
    }
}  
...
#debug concat("box_vertex_1 = (", vstr(3, box_vertex_1, ",", 0, -1), ")\n")   
#debug concat("box_vertex_2 = (", vstr(3, box_vertex_2, ",", 0, -1), ")\n")
#debug concat("box_vertex_3 = (", vstr(3, box_vertex_3, ",", 0, -1), ")\n")
#debug concat("box_vertex_4 = (", vstr(3, box_vertex_4, ",", 0, -1), ")\n")
#debug concat("box_vertex_5 = (", vstr(3, box_vertex_5, ",", 0, -1), ")\n")
#debug concat("box_vertex_6 = (", vstr(3, box_vertex_6, ",", 0, -1), ")\n")
#debug concat("box_vertex_7 = (", vstr(3, box_vertex_7, ",", 0, -1), ")\n")
#debug concat("box_vertex_8 = (", vstr(3, box_vertex_8, ",", 0, -1), ")\n")
